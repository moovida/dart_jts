...
//    symbols.setDecimalSeparator('.');
  ...
  /// @param  formatter       the <code>NumberFormatter</code> to use to convert
  ///      from a precise coordinate to an external coordinate
  void appendGeometryTaggedText6Args(Geometry geometry, List<Ordinate> outputOrdinates, bool useFormatting, int level,
  ...
  void appendPointTaggedText(Point point, List<Ordinate> outputOrdinates, bool useFormatting, int level,
      StringBuffer writer, NumberFormat formatter) {
    writer.write("POINT ");
  ...
  /// @param  formatter       the <code>NumberFormatter</code> to use to convert
  ///      from a precise coordinate to an external coordinate
  void appendLineStringTaggedText(LineString lineString, List<Ordinate> outputOrdinates, bool useFormatting, int level,
      StringBuffer writer, NumberFormat formatter) {
    writer.write("LINESTRING ");
    appendOrdinateText(outputOrdinates, writer);
    appendSequenceText(
        lineString.getCoordinateSequence(), outputOrdinates, useFormatting, level, false, writer, formatter);
  }
  ...
  /// @param  formatter       the <code>NumberFormatter</code> to use to convert
  ///      from a precise coordinate to an external coordinate
  void appendLinearRingTaggedText(LinearRing linearRing, List<Ordinate> outputOrdinates, bool useFormatting, int level,
      StringBuffer writer, NumberFormat formatter) {
    writer.write("LINEARRING ");
    appendOrdinateText(outputOrdinates, writer);
    appendSequenceText(
        linearRing.getCoordinateSequence(), outputOrdinates, useFormatting, level, false, writer, formatter);
  }
  ...
  /// @param  formatter       the <code>NumberFormatter</code> to use to convert
  ///      from a precise coordinate to an external coordinate
  void appendPolygonTaggedText(Polygon polygon, List<Ordinate> outputOrdinates, bool useFormatting, int level,
      StringBuffer writer, NumberFormat formatter) {
  ...
  /// @param  formatter       the <code>NumberFormatter</code> to use to convert
  ///      from a precise coordinate to an external coordinate
  void appendMultiPointTaggedText(MultiPoint multipoint, List<Ordinate> outputOrdinates, bool useFormatting, int level,
      StringBuffer writer, NumberFormat formatter) {
  ...
  /// @param  formatter       the <code>NumberFormatter</code> to use to convert
  ///      from a precise coordinate to an external coordinate
  void appendMultiLineStringTaggedText(MultiLineString multiLineString, List<Ordinate> outputOrdinates,
      bool useFormatting, int level, StringBuffer writer, NumberFormat formatter) {
  ...
  /// @param  formatter       the <code>NumberFormatter</code> to use to convert
  ///      from a precise coordinate to an external coordinate
  void appendMultiPolygonTaggedText(MultiPolygon multiPolygon, List<Ordinate> outputOrdinates, bool useFormatting,
      int level, StringBuffer writer, NumberFormat formatter) {
  ...
  /// @param  formatter       the <code>NumberFormatter</code> to use to convert
  ///      from a precise coordinate to an external coordinate
  void appendGeometryCollectionTaggedText(GeometryCollection geometryCollection, List<Ordinate> outputOrdinates,
      bool useFormatting, int level, StringBuffer writer, NumberFormat formatter) {
  ...
  static void appendCoordinate(
      CoordinateSequence seq, List<Ordinate> outputOrdinates, int i, StringBuffer writer, NumberFormat formatter) {
  ...
  /// @param  writer          the output writer to append to
  /// @param  formatter       the formatter to use for writing ordinate values.
  void appendSequenceText(CoordinateSequence seq, List<Ordinate> outputOrdinates, bool useFormatting, int level,
      bool indentFirst, StringBuffer writer, NumberFormat formatter) {
    if (seq.size() == 0) {
      writer.write("EMPTY");
    } else {
  ...
  /// @param  writer          the output writer to append to
  /// @param  formatter       the formatter to use for writing ordinate values.
  void appendPolygonText(Polygon polygon, List<Ordinate> outputOrdinates, bool useFormatting, int level,
      bool indentFirst, StringBuffer writer, NumberFormat formatter) {
    if (polygon.isEmpty()) {
      writer.write("EMPTY");
    } else {
  ...
  /// @param  writer          the output writer to append to
  /// @param  formatter       the formatter to use for writing ordinate values.
  void appendMultiPointText(MultiPoint multiPoint, List<Ordinate> outputOrdinates, bool useFormatting, int level,
      StringBuffer writer, NumberFormat formatter) {
    if (multiPoint.isEmpty()) {
      ...
      for (int i = 0; i < multiPoint.getNumGeometries(); i++) {
        ...
        appendSequenceText((multiPoint.getGeometryN(i) as Point).getCoordinateSequence(), outputOrdinates,
  ...
  /// @param  writer           the output writer to append to
  /// @param  formatter        the formatter to use for writing ordinate values.
  void appendMultiLineStringText(MultiLineString multiLineString, List<Ordinate> outputOrdinates, bool useFormatting,
      int level, /*bool indentFirst, */ StringBuffer writer, NumberFormat formatter) {
    if (multiLineString.isEmpty()) {
      ...
      for (int i = 0; i < multiLineString.getNumGeometries(); i++) {
        ...
        appendSequenceText((multiLineString.getGeometryN(i) as LineString).getCoordinateSequence(), outputOrdinates,
  ...
  /// @param  writer          the output writer to append to
  /// @param  formatter       the formatter to use for writing ordinate values.
  void appendMultiPolygonText(MultiPolygon multiPolygon, List<Ordinate> outputOrdinates, bool useFormatting, int level,
      StringBuffer writer, NumberFormat formatter) {
    if (multiPolygon.isEmpty()) {
      ...
      for (int i = 0; i < multiPolygon.getNumGeometries(); i++) {
        ...
        appendPolygonText(multiPolygon.getGeometryN(i) as Polygon, outputOrdinates, useFormatting, level2, doIndent,
            writer, formatter);
      }
      writer.write(")");
    }
  ...
  /// @param  writer          the output writer to append to
  /// @param  formatter       the formatter to use for writing ordinate values.
  void appendGeometryCollectionText(GeometryCollection geometryCollection, List<Ordinate> outputOrdinates,
      bool useFormatting, int level, StringBuffer writer, NumberFormat formatter) {
    if (geometryCollection.isEmpty()) {
      ...
      for (int i = 0; i < geometryCollection.getNumGeometries(); i++) {
        ...
        appendGeometryTaggedText6Args(
            geometryCollection.getGeometryN(i), outputOrdinates, useFormatting, level2, writer, formatter);
      }
      writer.write(")");
    }
  }
...
// create a sequence for one coordinate
    ...
    CoordinateSequence sequence =
        csFactory.createSizeDimMeas(1, toDimension(ordinateFlags), ordinateFlags.contains(Ordinate.M) ? 1 : 0);
...
// read close token if it was opened here
  ...
  CoordinateSequence getCoordinateSequenceTryParen(
    ...
    if (getNextEmptyOrOpener(tokenizer) == EMPTY)
      return this
          .csFactory
          .createSizeDimMeas(0, toDimension(ordinateFlags), ordinateFlags.contains(Ordinate.M) ? 1 : 0);
  ...
  CoordinateSequence mergeSequences(List<CoordinateSequence> sequences, List<Ordinate> ordinateFlags) {
    ...
    CoordinateSequence sequence = this
        .csFactory
        .createSizeDimMeas(sequences.length, toDimension(mergeOrdinates), mergeOrdinates.contains(Ordinate.M) ? 1 : 0);
...
//        return "End-of-Line";
  ...
  MultiPoint readMultiPointText(WKTTokenizer tokenizer, List<Ordinate> ordinateFlags) {
    return geometryFactory.createMultiPointSeq(
        getCoordinateSequenceTryParen(tokenizer, ordinateFlags, this.isAllowOldJtsMultipointSyntax));
  }
