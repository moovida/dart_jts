...
class OverlayUtil {
  ...
  static Envelope? clippingEnvelope(
      int opCode, InputGeometry inputGeom, PrecisionModel pm) {
    Envelope? resultEnv = resultEnvelope(opCode, inputGeom, pm);
    if (resultEnv == null) return null;

    Envelope clipEnv = RobustClipEnvelopeComputer.getEnvelopeExpanded(
        inputGeom.getGeometry(0)!, inputGeom.getGeometry(1)!, resultEnv);

    Envelope safeEnv = safeEnvelope(clipEnv, pm);
    return safeEnv;
  }
  ...
  static Envelope? resultEnvelope(
    ...
    switch (opCode) {
      case OverlayNG.INTERSECTION:
        ...
        Envelope envA = safeEnvelope(inputGeom.getEnvelope(0)!, pm);
        Envelope envB = safeEnvelope(inputGeom.getEnvelope(1)!, pm);
        overlapEnv = envA.intersection(envB);
        break;
      case OverlayNG.DIFFERENCE:
        overlapEnv = safeEnvelope(inputGeom.getEnvelope(0)!, pm);
        break;
    }
   ...
   */
  static Envelope safeEnvelope(Envelope env, PrecisionModel pm) {
    double envExpandDist = safeExpandDistance(env, pm);
    Envelope safeEnv = env.copy();
    safeEnv.expandByDistance(envExpandDist);
    return safeEnv;
  }

  static final double SAFE_ENV_BUFFER_FACTOR = 0.1;

  static final int SAFE_ENV_GRID_FACTOR = 3;
  ...
  static bool isEmptyResult(
      int opCode, Geometry? a, Geometry? b, PrecisionModel pm) {
    switch (opCode) {
      case OverlayNG.INTERSECTION:
  ...
  static Geometry? createEmptyResult(int dim, GeometryFactory geomFact) {
    ...
    switch (dim) {
      case 0:
        result = geomFact.createPointEmpty();
        break;
      case 1:
        result = geomFact.createLineStringEmpty();
        break;
      case 2:
        result = geomFact.createPolygonEmpty();
        break;
      case -1:
        result = geomFact.createGeometryCollectionEmpty();
        break;
      default:
  ...
  static Geometry createResultGeometry(
    ...
    if (resultPolyList.isNotEmpty) geomList.addAll(resultPolyList);
    if (resultLineList.isNotEmpty) geomList.addAll(resultLineList);
    if (resultPointList.isNotEmpty) geomList.addAll(resultPointList);
  ...
  static String labelForResult(OverlayEdge edge) {
    return edge.getLabel().toStringLocal(edge.isForward()) +
   ...
   */
  static Coordinate? roundPoint(Point pt, PrecisionModel pm) {
    if (pt.isEmpty()) return null;
    return roundCoordinate(pt.getCoordinates()[0], pm);
  }
   ...
   */
  static Coordinate? roundCoordinate(Coordinate p, PrecisionModel pm) {
    if (!isFloating(pm)) {
      Coordinate pRound = p.copy();
      pm.makeCoordinatePrecise(pRound);
      return pRound;
    }
    return p;
  }

  static final double AREA_HEURISTIC_TOLERANCE = 0.1;
    ...
    switch (opCode) {
      ...
      case OverlayNG.DIFFERENCE:
        isConsistent = isDifferenceAreaConsistent(
            areaA, areaB, areaResult, AREA_HEURISTIC_TOLERANCE);
        break;
      case OverlayNG.SYMDIFFERENCE:
        isConsistent =
